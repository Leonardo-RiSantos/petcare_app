...
export default function TutorChatScreen({ route, navigation }) {
  ...
  useEffect(() => {
    if (!petId) return;
    navigation.setOptions({ title: vetName ? `Dr(a). ${vetName.split(' ')[0]}` : 'Chat com Vet' });
    loadMessages();

    const channel = supabase
      .channel(`chat:${petId}:${vetId}`)
      .on('postgres_changes', {
        event: 'INSERT',
        schema: 'public',
  ...
  const loadMessages = async () => {
    ...
    // Marca mensagens do vet como lidas
    await supabase.from('vet_chat_messages')
      .update({ read_at: new Date().toISOString() })
      .eq('vet_id', vetId).eq('pet_id', petId)
      .eq('sender_role', 'vet').is('read_at', null);
  };
  ...
  const sendMessage = async () => {
    ...
    if (!error) {
      sendPushToUser(vetId, {
        title: `Mensagem de ${petName || 'tutor'}`,
        body: content,
        ...
        ListEmptyComponent={
          <View style={styles.emptyWrap}>
            ...
            <Text style={styles.emptySub}>
              {vetName ? `Inicie a conversa com Dr(a). ${vetName.split(' ')[0]}` : 'Envie a primeira mensagem'}
            </Text>
          </View>
        }
      />
...
const styles = StyleSheet.create({
  ...
  vetLabel: { fontSize: 10, fontWeight: '700', color: '#0EA5E9', marginBottom: 3 },
